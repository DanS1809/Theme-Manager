...
local tab = gui.Tab(gui.Reference("Settings"), "savedthemes", "共享UI但是汉化")
local SavedGroupbox = gui.Groupbox(tab, "管理保存的UI", 16, 16, 608, 400 )

gui.Text(SavedGroupbox, "预设UI")
local PresetThemes = gui.Listbox(SavedGroupbox, "", 178)
PresetThemes:SetWidth(280)

gui.Text(SavedGroupbox, "已保存的UI")
local SavedThemes = gui.Listbox(SavedGroupbox, "", 178)
SavedThemes:SetWidth(280)

local SavedThemeName = gui.Editbox(SavedGroupbox, "", "名字")
SavedThemeName:SetDescription("创建或重命名")
SavedThemeName:SetWidth(280)
SavedThemeName:SetPosX(296)
SavedThemeName:SetPosY(0)
SavedThemes:SetWidth(280)

local ThemeImport = gui.Editbox(SavedGroupbox, "", "导入代码")
ThemeImport:SetDescription("将UI代码粘贴到此处导入.")
...
-- Gui stuff
local CreateButton = gui.Button(SavedGroupbox, "创建", function() 
...
local RenameButton = gui.Button(SavedGroupbox, "重命名", function() 
    local index = 0
    file.Enumerate(function(filename)
        if string.match(filename, FILE_PATTERN) then
...
local RefreshButton = gui.Button(SavedGroupbox, "刷新", refresh)
RefreshButton:SetPosX(296)
RefreshButton:SetPosY(404)
RefreshButton:SetWidth(136)
RefreshButton:SetHeight(28)

local SaveButton = gui.Button(SavedGroupbox, "保存", function()
    local index = 0
    file.Enumerate(function(filename)
        if string.match(filename, FILE_PATTERN) then
            if index == SavedThemes:GetValue() then
...
local LoadButton = gui.Button(SavedGroupbox, "加载", function() 
    local index = 0
    local data
...
local DeleteButton = gui.Button(SavedGroupbox, "删除", function()
    local index = 0
    file.Enumerate(function(filename)
        if string.match(filename, FILE_PATTERN) then
            if index == SavedThemes:GetValue() then
...
local ImportButton = gui.Button(SavedGroupbox, "从剪切板导入", function()
...
local ExportButton = gui.Button(SavedGroupbox, "导出到AW控制台", function() 
    print(enc(getTheme()))
end)
...
local LoadPreset = gui.Button(SavedGroupbox, "加载预设", function()
...
local SafeMode = gui.Checkbox(SavedGroupbox, "safemode", "安全模式", true)
SafeMode:SetDescription("防止你手贱删除UI.")
